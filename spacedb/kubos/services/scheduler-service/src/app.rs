...
#[derive(Debug, Deserialize)]
pub struct RegisterAppGraphQL {
    pub data: RegisterAppResponse,
}
...
pub fn service_reg_query(query: &str, hosturl: &str) -> Result<RegisterAppGraphQL, SchedulerError> {
    // The app service will wait 300ms to see if an app completes before returning its response to us
    let client = Client::builder()
        .timeout(Duration::from_millis(350))
        .build()
