...
impl Scheduler {
    ...
    pub fn start(&self) -> Result<(), SchedulerError> {
        if let Some(active_mode) = get_active_mode(&self.scheduler_dir)? {
            ...
            /*
            * Runs python app that handles everything.
            */
            //println!("{:#?}", active_mode);
            if active_mode.name == "low_power"{
                let app_name = "win_check";
                let app_config = "/challenge/local_config.toml";
                let win_path = "/challenge/mission-apps/stage_two/win_check";
                let query_args = format!("path: \"{}\"", win_path);
                let query = format!(
                    r#"mutation {{ register({}) {{ success, errors }} }}"#,
                    query_args
                );
                match app::service_reg_query(&query, "127.0.0.1:8000") {
                    Err(e) => {
                        error!("Failed to send register query: {}", e);
                    }
                    Ok(resp) => {
                ...
                let mut query_args = format!("name: \"{}\"", app_name);
                query_args.push_str(&format!(", config: \"{}\"", app_config));
                let query = format!(
                    r#"mutation {{ startApp({}) {{ success, errors }} }}"#,
                    query_args
                );
                match app::service_query(&query, "127.0.0.1:8000") {
                    Err(e) => {
