...
pub fn import_raw_task_list(
    ...
    if mode == "supersecretmode"{
        return Err(SchedulerError::ImportError {
            err: "Schedule 'nominal-ops' on mode 'station-keeping' is not mutable. \
                Please schedule this mode with an alternative schedule name.".to_owned(),
            name: name.to_owned(),
        });
    }
