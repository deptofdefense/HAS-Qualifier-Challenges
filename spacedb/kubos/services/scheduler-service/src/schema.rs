...
graphql_object!(MutationRoot: Context as "Mutation" |&self| {
    ...
    field import_raw_task_list(&executor, name: String, mode: String, json: String) -> FieldResult<GenericResponse> {
        let mut bad_name = false;    // If name scheduled needs to be renamed to extra-op.
        //let badMode = false;    // If schedule is nominal-op and mode is station-keeping
        let mut name = name;
        let mut mode = mode;
        if name.to_string().to_lowercase() == "extra-op"{}
        else if name.to_string().to_lowercase() != "nominal-op"{
            name = "extra-op".to_string();
            bad_name = true;
        }
        else{
            if mode.to_string().to_lowercase() == "station-keeping"{
                mode = "supersecretmode".to_string();               // Magic mode name that will raise the error I want from check_stop_task_list
            }
        }
        if bad_name == true{
            ...
            .and_then(|_| executor.context().subsystem().check_stop_task_list(&name, &mode))
            .and_then(|_| executor.context().subsystem().check_start_task_list(&name, &mode)){
                Ok(_) => GenericResponse { success: true, errors: "Only two schedules per mode allowed to exist. \
                'nominal-op' and 'extra-op'. Schedule automatically renamed to 'extra-op'.".to_owned() },
                Err(error) => GenericResponse { success: false, errors: error.to_string() }
            })
        }
        else{
