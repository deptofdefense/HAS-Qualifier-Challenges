...
if __name__ == '__main__':
    ...
    ticket = 'ticket{delta60071lima:GG6gO3M79yLG2DHNBJy_sGPgEukO_ZQkuaHocmIGb1sZEacx5oB_jHIhr_IA7EnZGg}'
    Host = 'spacerace.satellitesabove.me'
    Port = 5063
    ...
    #LCM - re-enable for production testing
    line = sock.recv(128)
    # sock.close()
    ...
    # Setup the UDP listener for telemetry data
    # tlmsock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    
    # try:
    #     tlmsock.bind(('0.0.0.0', UDP_LISTEN_PORT))

    # except:
    #     print("Unable to bind for UDP listener")
    ...
    appsend = cmdSender(0, cmdsock)

    # send this to get a process list
    # hksend.send(b'\x65')

    # configure the telemetry sender to send UDP packets to the listener
    # tlmsend.send(b'\x00' + myhostip.encode('utf-8') + ':{}'.format(UDP_LISTEN_PORT).encode('utf-8'))
