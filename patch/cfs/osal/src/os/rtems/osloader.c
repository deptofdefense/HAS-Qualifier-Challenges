 ...
 *
 *-----------------------------------------------------------------*/
static bool OS_rtems_rtl_check_unresolved (rtems_rtl_unresolv_rec* rec,
        void*                     data)
{
    int32 *status = data;
    ...
    switch(rec->type)
    {
    case rtems_rtl_unresolved_empty: /**< The record is empty. Must always be 0 */
        OS_DEBUG("rtems unresolved symbol empty; %s\n", rec->rec.name.name);
        // *status = OS_ERROR;
        break;
    case rtems_rtl_unresolved_symbol: /**< The record is a symbol. */
        OS_DEBUG("rtems unresolved symbol: %s\n", rec->rec.name.name);
        *status = OS_ERROR;
        break;
    case rtems_rtl_unresolved_reloc: /**< The record is a relocation record. */
        OS_DEBUG("rtems check symbol relocation record: %s\n", rec->rec.name.name);
        // *status = OS_ERROR;
        break;
    case rtems_rtl_trampoline_reloc: /**< The record is a trampoline relocation record. */
        OS_DEBUG("rtems check symbol trampoline relocation record: %s\n", rec->rec.name.name);
        // *status = OS_ERROR;
        break;
    default:
        break;
...
int32 OS_ModuleLoad_Impl ( uint32 module_id, char *translated_path )
{
    ...
    else if (unresolved)
    {
         ...
         */
        OS_DEBUG("Module has unresolved externals: %s\n", translated_path);
        // OS_DEBUG("module has has unresolved externals\n");
        status = OS_SUCCESS; /* note - not final, probably overridden */
        rtems_rtl_unresolved_iterate (OS_rtems_rtl_check_unresolved, &status);
    }
    else
    {
        status = OS_SUCCESS;
