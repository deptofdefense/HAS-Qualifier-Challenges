...
// Believe this should be fixed. Cosmos is sending the Cmd secondary header reversed. I believe this is primarily
// to accomodate Little Endian targets. So for Big Endian targets, we have to reverse the order to match Cosmos
// Ideally Cosmos should send the secondary header in the appropriate order FC | XSUM and we should reverse it here
// for Little Endian targets.  I'm concerned that there may be farther reaching impacts, so I'm leaving for now
#ifdef SOFTWARE_BIG_BIT_ORDER
  ...
  /* Write function code to command secondary header. */
  #define CCSDS_WR_FC(shdr,value)     CCSDS_WR_BITS((shdr).Command, 0x007F, 0, value)

  /* Read checksum from command secondary header. */
  #define CCSDS_RD_CHECKSUM(shdr)     CCSDS_RD_BITS((shdr).Command, 0xFF00, 8)
  /* Write checksum to command secondary header. */
  #define CCSDS_WR_CHECKSUM(shdr,val) CCSDS_WR_BITS((shdr).Command, 0xFF00, 8, val)
#endif
