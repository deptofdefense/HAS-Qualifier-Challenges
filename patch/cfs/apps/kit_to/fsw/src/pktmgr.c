...
void PKTMGR_Constructor(PKTMGR_Class*  PktMgrPtr, char* PipeName, uint16 PipeDepth)
{
   ...
   CFE_PSP_MemCpy(PktMgr->TlmKey,get_key(),32);
   CFE_PSP_MemCpy(PktMgr->TlmIV,get_iv(),16);
...
int PKTMGR_encrypt(unsigned char *plaintext, int plaintext_len, unsigned char *key,
            unsigned char *iv, unsigned char *ciphertext)
{
  ...
  CFE_PSP_MemCpy(ciphertext, iv, 16);
  ciphertext += 16;
  ciphertext_len += 16;
...
void PKTMGR_FillData(unsigned char *buff)
{
   unsigned char *ptr = buff;
   unsigned char ii = 0;
   for (; ptr != buff+0x400; ptr++,ii++)
   {
      *ptr = ii;
   } 
}
...
void PKTMGR_OutputTelemetry(void)
{
   ...
   uint16                    PktLen;
   CFE_SB_Msg_t              *PktPtr;
   unsigned char             paddingBuff[RANDOM_SIZE_PATCH];
   ...
   do
   {
      ...
      if ( (SbStatus == CFE_SUCCESS) && (PktMgr->SuppressSend == FALSE) ) {
         ...
         if(PktMgr->DownlinkOn) {
            ...
            int len = PKTMGR_encrypt((unsigned char*)PktPtr, PktLen, PktMgr->TlmKey, PktMgr->TlmIV, ciphertext);
               
            SocketStatus = sendto(PktMgr->TlmSockId, (char *)ciphertext, len, 0,
