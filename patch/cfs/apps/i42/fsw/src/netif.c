...
int32 NETIF42_Recv(char* BufPtr, const uint16 BufSize) 
{
   ...
   // Populate Server structure in support of socket read.  Should really go in contstructor.  Leaving for now
   // This isn't the implelementation we'll likely keep moving forward
   memset((char *) &Server,0,sizeof(Server));
   Server.sin_family = AF_INET;
   Server.sin_addr.s_addr = inet_addr(I42_SOCKET_ADDR_STR);
   // memcpy((char *)&Server.sin_addr.s_addr,(char *)Host->h_addr_list[0], Host->h_length);
   Server.sin_port = htons(NetIf->Port);
...
static boolean InitSocket(const char *HostName, uint16 Port, boolean AllowBlocking)
{
   ...
   // Get hostbyname not supported in rtems osal
