...
{
  /*  cmd time,  <---------------------------- cmd pkt primary header ---------------------------->  <----- cmd pkt 2nd header ---->   <-- opt data ---> */
      DELAY_0, CFE_MAKE_BIG16(ISIM_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD_LEN_NO_PARAM), ((ISIM_STOP_SCI_XSUM << 8) | ISIM_STOP_SCI_CMD_FC),
      DELAY_4, CFE_MAKE_BIG16(ISIM_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD_LEN_NO_PARAM), ((ISIM_PWR_OFF_XSUM  << 8) | ISIM_PWR_OFF_CMD_FC)
};
#endif
/************************/
/*  End of File Comment */
