...
{
  /*  cmd time,  <---------------------------- cmd pkt primary header ---------------------------->  <----- cmd pkt 2nd header ---->   <-- opt data ---> */
     CMD1_TIME,  CFE_MAKE_BIG16(SIM_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD1_LENGTH), ((CMD1_XSUM << 8) | SIM_HEATER_MISBEHAVE_CC), 0,
     CMD2_TIME,  CFE_MAKE_BIG16(LC_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD2_LENGTH), ((CMD2_XSUM << 8) | LC_SET_AP_STATE_CC), 0, 1,
     CMD3_TIME,  CFE_MAKE_BIG16(LC_CMD_MID), CFE_MAKE_BIG16(PKT_FLAGS), CFE_MAKE_BIG16(CMD3_LENGTH), ((CMD3_XSUM << 8) | LC_SET_AP_STATE_CC), 1, 1
};
#endif
/************************/
/*  End of File Comment */
