                 ...
                 uint32       DestAddress)
{
   ...
   for (size_t ii = 0; ii < BytesProcessed; ii++)
   {
      if (MM_AppData.BytesModified >= WRITE_LIMIT) 
      {
         CFE_EVS_SendEvent(MM_PSP_WRITE_ERR_EID, CFE_EVS_ERROR,
                        "PSP write memory error: Too many bytes written, game over!");
         ValidPoke = FALSE;
         break;
      }

      uint8 value = 0xFF & (WordValue >> (8*ii));
      PSP_Status = CFE_PSP_MemWrite8(DestAddress + ii, value);
      if (PSP_Status != CFE_PSP_SUCCESS)
      {
         MM_AppData.ErrCounter++;
         CFE_EVS_SendEvent(MM_PSP_WRITE_ERR_EID, CFE_EVS_ERROR,
                        "PSP write memory error: RC=0x%08X, Address=0x%08X, MemType=%s", 
                           (unsigned int)PSP_Status, (unsigned int)DestAddress, MemTypeStr);
         ValidPoke = FALSE;
         break;
      }
      
      MM_AppData.BytesModified++;
   }
   ...
   if (ValidPoke)
   {
      ...
      MM_AppData.DataValue  = WordValue;
      MM_AppData.BytesProcessed = BytesProcessed;

      /* Challenge Modification */
      CFE_EVS_SendEvent(MM_POKE_WORD_INF_EID, CFE_EVS_INFORMATION,
                        "Poke Command: Addr = 0x%08X, Size = %d bits, Data = 0x%04X",
                           (unsigned int)DestAddress, CmdPtr->DataSize, WordValue);
   }
