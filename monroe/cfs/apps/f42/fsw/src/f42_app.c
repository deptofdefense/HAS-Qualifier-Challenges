...
static void SendCtrlTlm(void)
{
   ...
   for (i=0; i<3; i++) {
      F42CtrlPkt.wbn[i]    = F42.F42Adp.Fsw.wbn[i];
      F42CtrlPkt.qbr[i]    = F42.F42Adp.Fsw.qbr[i];
      F42CtrlPkt.AttErr[i] = F42.F42Adp.Fsw.qbr[i]*2.0;
      F42CtrlPkt.Herr[i]   = F42.F42Adp.Fsw.ierr[i];  // TODO - Hack to get Her to TLM
      F42CtrlPkt.svb[i]    = F42.F42Adp.Fsw.svb[i];
      F42CtrlPkt.WhlCmd[i] = F42.F42Adp.Fsw.Twhlcmd[i];
      F42CtrlPkt.MtbCmd[i] = F42.F42Adp.Fsw.Mmtbcmd[i];
   }
   F42CtrlPkt.qbr[3]   = F42.F42Adp.Fsw.qbr[3];
   F42CtrlPkt.GimCmd   = F42.F42Adp.Fsw.GimCmd[0].Ang[0];
